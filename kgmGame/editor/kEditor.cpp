...
void kEditor::select(kgmString name)
{
  ...
  for(i = nodes.begin(); i != nodes.end(); ++i)
  {
    if((*i)->getName() == name)
    {
      ...
      if(pivot && selected)
      {
        ((kPivot*)pivot->getMesh())->setPos((*i)->position());
...
void kEditor::select(int x, int y)
{
  ...
  if(peeked)
  {
    ...
    if(pivot)
      ((kPivot*)pivot->getMesh())->peekAxis(ray);
  }
  else
  {
    dragging = null;
...
void kEditor::onMsMove(int k, int x, int y)
{
  ...
  if(game->getRender())
  {
    kgmMesh* mesh = pivot->getMesh();

    kPivot *pvt = (kPivot *) mesh;
  ...
  else if(dragging && pivot && paxes != kPivot::AXIS_NONE && ms_click[0] && !dragging->lock())
  {
    ...
    m.identity();
    m.translate(pos);
    ((kPivot*)pivot->getMesh())->setPos(pos);
    pivot->set(m);
  }
}
