...
inline bool ptInPolygon(vec3& pt, vec3* poly, uint points){
 double angle = 0.0;
...
inline bool crossEllipticPolygon(vec3& pt, 
								 float rx, float ry, float rz,
								 vec3* poly, uint points){
 for(int i = 0; i < points; i++){
  vec3  vs;
  vec3  ve;
  vec3  pr;
...
bool kgmCollision::collision(vec3& start, vec3& end, float radius, 
							 vec3& tra, vec3& trb, vec3& trc, vec3& pt_insect){
...
bool kgmCollision::collision(vec3& start, vec3& end, 
							 float rx, float ry, float rz, box3& b, mtx4& btr){
 int i = 0;
 vec3 box_points[8];
 vec3 box_sides[6][4];
 b.points(box_points);
