...
//RENDER
 void  gcSetParameter(uint param, void* value);
 void  gcGetParameter(uint param, void* value);
 void  gcClear(uint flag, uint col, float depth, uint sten);
 void  gcBegin();
 void  gcEnd();
 void  gcRender();
...
//MODE
 ...
 void  gc3DMode();
//DRAW
 void  gcDraw(uint pmt, uint v_fmt, uint v_size, uint v_cnt, void *v_pnt, uint i_size, uint i_cnt, void *i_pnt);
...
//LIGHT & MATERIAL
 //void  gcSetLight(int i, kgmLight *light);
 //void  gcSetMaterial(kgmMaterial *m);
//FOG
 //void  gcFog(kgmFog* fog);
 //BLEND
