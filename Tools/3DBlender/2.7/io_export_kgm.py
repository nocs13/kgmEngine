...
kPi = 3.14159265358979323846264338328
...
class kgmMesh:
    ...
    if mesh_faces:
      ...
      for i in range(0, len(mesh_faces)):
        ...
        for j in range(0, len(face.vertices)):
          ...
          n = face.normal
          v.v = [kRound(c[0]), kRound(c[1]), kRound(c[2])]
          v.n = [kRound(n[0]), kRound(n[1]), kRound(n[2])]
  ...
  def addVertex(self, vx):
    ...
    print('Adding vertex vx: {0}, {1}, {2}'.format(vx.v[0], vx.v[1], vx.v[2]))

    # check if such vertex already exist

    for i in range(0, len(self.vertices)):
      v = self.vertices[i]
      if (kEqual(v.v[0], vx.v[0]) and kEqual(v.v[1], vx.v[1]) and kEqual(v.v[2], vx.v[2])):
