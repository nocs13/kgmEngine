...
class kgmBoneAnimation(kgmAnimation):
 def __init__(self, o, a):
  ...
  for frame in range(0, frameCount):
   bpy.context.scene.frame_set(frame + bpy.context.scene.frame_start)
   pos = mathutils.Vector((0, 0, 0)) * o.bone.matrix_local
   rot = o.bone.matrix_local.to_quaternion()
...
class kgmExport(bpy.types.Operator):
 ...
 exp_animations = BoolProperty(name="Export Animations", description="", default=True)
