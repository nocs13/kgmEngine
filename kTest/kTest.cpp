...
public:
  ...
  bool action(kgmObject* o, int a){
    ...
    if(o->instanceof(kgmCamera::Class)){
      ...
      kgmCamera::cast(o)->rotate(rotate.x, rotate.y, rotate.z);
    }
    return true;
  }
};
...
public:
  ...
  bool action(kgmObject* o, int a){
    ...
    if(o->instanceof(kgmCamera::Class)){
      kgmCamera::cast(o)->rotate(a * rotate.x, a * rotate.y, a * rotate.z);
      fprintf(stderr, "\nLOL %i", a);
    }
    return true;
  }
...
public:
  ...
  :kgmGameLogic::Sensor(o){
    ...
    ctl->add(new kActMotion(.0001, vec3(0, 0, 0)));
    ctls.add(ctl);
    ctl = new kgmGameLogic::Controller(kgmGameLogic::ControllerAnd);
    ctl->add(new kActMotion(-.0001, vec3(0, 0, 0)));
    ctls.add(ctl);  
    ...
    ctls.add(ctl);  
    ctl = new kgmGameLogic::Controller(kgmGameLogic::ControllerAnd);
    ctl->add(new kActRotation(vec3(0.0,  0.0, 0.001)));
    ctls.add(ctl);
    //ctl = new kgmGameLogic::Controller(kgmGameLogic::ControllerAnd);
    //ctl->add(new kActRotation(vec3(0.0001, 0.0, 0.0)));
    //ctls.add(ctl);
  }
...
class kGame: public kgmGameBase{
  ...
  public:
   ...
   :kgmGui(0, x, y, w, h){
    ((kGame*)kgmIGame::getGame())->m_guis.add(this);
   }
...
public:
  ...
  int gLoad(kgmString s){
    ...
    //if(m_map.camera){
    //  ((kLogic*)m_logic)->addFlyMotion(m_map.camera);
    //  fprintf(stderr, "\nActive camera exist");
    //}
    return r;
  }
  ...
  static int luaGui(lua_State* lua){
   ...
   gui->setFont(((kGame*)kgmIGame::getGame())->m_font);
   lua_pushlightuserdata(lua, gui);
   return 1;
  }
  ...
  static int luaSetGuiColor(lua_State* lua){
   ...
   gui->setColor(cid, lua_tointeger(lua, 3));
   return 0;
  }
